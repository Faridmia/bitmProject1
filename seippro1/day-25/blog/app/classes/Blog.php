...
class Blog {
    ...
    public function updateBlogData($data,$file){
        ...
        if(isset($file['blog_image']['name'])){
            ...
            if(!empty(isset($file['blog_image']['name']) && !empty($file['blog_image']['name']))) {
                $select = "SELECT * FROM blog WHERE b_id = '$bid'";
                $queryResult = mysqli_query(Database::dbConnection(),$select);
                $blogInfo = mysqli_fetch_assoc($queryResult);
                //$delimage = $blogInfo['blog_image'];
                unlink('../assets/upload/blog_image/'.$blogInfo['blog_image']);
                $newFile = md5(uniqid(rand(), true)) . '.' . $extension;
